...
struct DoUntilState : public ContinuationBase<> {
  DoUntilState(Stop &&stop, Func &&func) : stop_(std::forward<Stop>(stop)), function_(std::forward<Func>(func)) {}
...
Future<> DoUntil(Stop &&stop, Func &&func) {
  ...
  do {  // Fast path.
    ...
    if (future.Ready())
      ...
      auto state = new details::DoUntilState<Stop, Func>(std::forward<Stop>(stop), std::forward<Func>(func));
