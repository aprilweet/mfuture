...
class Semaphore {
 public:
  ...
  Semaphore(std::size_t initial, std::size_t limit, Policy policy = Policy::kFifo)
    ...
    if (policy_ == Policy::kFifo) {
      ...
      waiters_ = std::make_unique<FastPolicyStore>(initial);
    }
  }
  ...
  void Post(std::size_t nr) {
    waiters_->Available() += nr;
    assert(waiters_->Available() <= limit_);

    if (!waiters_->Size()) return;
    return waiters_->Post(nr);
  }

  std::size_t Available() const { return waiters_->Available(); }
 ...
 private:
  ...
  struct PolicyStore {
    PolicyStore(std::size_t available) : available_(available) {}
