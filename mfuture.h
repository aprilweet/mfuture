  ...
  static void Resolve(Callback&& cb, V&& v, PromiseType&& pr) {
    ...
    try {
      if constexpr (std::is_void_v<R>) {
        ...
        promise.SetValue(std::move(r));  // FIXME: noexcept?
      } else {
        auto r = std::apply(std::forward<Callback>(cb), std::forward<V>(v));
        r.Fold(promise);
      }
...
class FutureState {
 ...
 public:
  ...
  // TODO(quintonwang): We should simplify this while it's resolved to reduce
...
// deduction with U&&.
//
// FIXME: Is it valid to declare multiple template param pack? Reference to
// seastar::make_ready_future. Ref:
...
// TODO(quintonwang): Add executor support, caring about the thread-safety
